    ...
    <SectionWrapper id={'generate'} header={"generate your workout"} title={['It\'s', 'Huge', 'o\'clock']}>
      ...
      {Object.keys(WORKOUTS).map((type, typeIndex) => {
        return (
          <button onClick={() => {
            setMuscles([])
            setPoison(type)
          }} className={'bg-slate-950 border border-blue-400 duration-200 px-4 hover:border-blue-600 py-3 rounded-lg' + (type === poison ? ' border-blue-800' : ' border-blue-400')} key={typeIndex}>
      ...
      {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
        return (
          <button onClick={() => {
            setGoal(scheme)
          }} className={'bg-slate-950 border border-blue-400 duration-200 px-4 hover:border-blue-600 py-3 rounded-lg' + (scheme === goal ? ' border-blue-800' : ' border-blue-400')} key={schemeIndex}>
